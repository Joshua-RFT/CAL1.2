    ...
    def retrieve(self, mock=False):
        ...
        token_counter = TokenCountingHandler(
            ...
            tokenizer=tiktoken.encoding_for_model("text-embedding-3-small").encode
                        #tokenizer=tiktoken.encoding_for_model("text-embedding-v3").encode

        )
        print('mook测试',mock,'mock测试')
        if not os.path.exists(persist_dir) or mock:
            ...
            if mock:
                ...
                Settings.callback_manager = CallbackManager([token_counter])
                                ...
                                #deyudada emb
                                #deyudada:一定要加这个api_base，原项目里面没写改不了url
                print('########else测试#########3')
                embed_model = OpenAIEmbedding(
                    ...
                    embed_batch_size=5
                                ...
                                #embed_model = OpenAIEmbedding(model_name="text-embedding-v3")

            index = VectorStoreIndex.from_documents(documents, embed_model=embed_model)
            index.storage_context.persist(persist_dir=persist_dir)
        else:
            storage_context = StorageContext.from_defaults(persist_dir=persist_dir)
            #print('storage_context测试',storage_context,'storage_context测试')
        ...
        print('###########')
        print(retriever.retrieve(self.problem_statement))
