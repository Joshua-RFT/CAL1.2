#Deyudada:change the path to import
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))
print("环境变量测试", os.environ["PROJECT_FILE_LOC"])
...
##from ..util.utils import load_existing_instance_ids, load_jsonl, setup_logger
...
def localize_instance(
    ...
    if args.fine_grain_line_level:#6进分支
        if len(found_files) != 0:
            ...
            for _ in range(MAX_RETRIES):
                ...
                if not args.direct_edit_loc:#6进分支
                ...
                #         # 将 additional_artifact_loc_edit_location 放入一个列表中
                #     additional_artifact_loc_edit_location = [additional_artifact_loc_edit_location]
...
def localize_irrelevant(args):
    ...
    print(swe_bench_data)
    print('=======================swe_bench_data=======================')
    # Access the first instance (row)
    first_instance = swe_bench_data[0]

    # Print the detailed content of the first instance
    print('=======================First Instance Details=======================')
...
#                 )
#                 print(merged_found_locs,'测试merge_found_locs@@@@@@@@@@@@@@@@')
#                 '''
#                 {'django/conf/global_settings.py': ['\nline: 307'], 'django/core/files/storage.py': ['\nline: 178\nline: 217\nline: 260\nline: 283']} 测试merge_found_locs@@@@@@@@@@@@@@@@
# {} 测试merge_found_locs@@@@@@@@@@@@@@@@
# {} 测试merge_found_locs@@@@@@@@@@@@@@@@
# {} 测试merge_found_locs@@@@@@@@@@@@@@@@
#
